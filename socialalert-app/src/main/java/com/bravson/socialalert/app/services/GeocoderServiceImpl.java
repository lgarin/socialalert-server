...
public class GeocoderServiceImpl implements GeocoderService {
	...
	public GeoArea decodeGeoHash(String geoHash) {
		...
		double distance = DistanceUtils.toRadians(degree) * DistanceUtils.EARTH_MEAN_RADIUS_KM / 2D;
		//double distance = DistanceUtils.degrees2Dist(Math.sqrt(rect.getArea(SpatialContext.GEO)), DistanceUtils.EARTH_MEAN_RADIUS_KM);
		return new GeoArea(center.getY(), center.getX(), distance);
	}
	
	public int computeGeoHashLength(GeoArea area) {
		double degree = DistanceUtils.dist2Degrees(area.getRadius(), DistanceUtils.EARTH_MEAN_RADIUS_KM * 2D);
